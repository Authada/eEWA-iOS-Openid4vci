  ...
  public init(from decoder: Decoder) throws {
    ...
    for (key, value): (String, JSON) in json {
      if let dictionary = value.dictionary,
        ...
        case SeTlvVcFormat.FORMAT:
            let profile = try SeTlvVcFormat.CredentialConfiguration(json: credJson)
            mapIdentifierCredential[credentialIdentifier] = .seTlvVc(profile)
